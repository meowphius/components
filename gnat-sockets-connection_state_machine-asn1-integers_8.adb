...
package body GNAT.Sockets.Connection_State_Machine.ASN1.Integers_8 is
   ...
   begin
      if Data (Pointer) <= 127 then
         Item.Value := Integer_8 (Data (Pointer));
      else
         Item.Value := -1 - Integer_8 (not Data (Pointer));
      end if;
      Pointer := Pointer + 1;
      State   := 0;
   end Feed;
   ...
   begin
      ...
      if Data (Pointer) <= 127 then
         Value := Integer_8 (Data (Pointer));
      else
         Value := - Integer_8 (not Data (Pointer)) - 1;
      end if;
      Pointer := Pointer + 1;
   end Get;
   ...
   begin
      ...
      if Value >= 0 then
         ...
         Data (Pointer) := not Stream_Element (-1 - Value);
      end if;
      Pointer := Pointer + 1;
   end Put;
