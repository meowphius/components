...
package body GNAT.Sockets.Connection_State_Machine.
   ...
   begin
      if State > 1 then
         Raise_Exception
         (  Data_Error'Identity,
            "Protocol error reading 8-bit little endian unsigned"
         );
      elsif State = 0 then
         Item.Value := 0;
         State      := 1;
   ...
   begin
      if State > 2 then
         Raise_Exception
         (  Data_Error'Identity,
            "Protocol error reading 16-bit little endian unsigned"
         );
      elsif State = 0 then
         Item.Value := 0;
         State      := 2;
   ...
   begin
      if State > 4 then
         Raise_Exception
         (  Data_Error'Identity,
            "Protocol error reading 32-bit little endian unsigned"
         );
      elsif State = 0 then
         Item.Value := 0;
         State      := 4;
   ...
   begin
      if State > 8 then
         Raise_Exception
         (  Data_Error'Identity,
            "Protocol error reading 64-bit little endian unsigned"
         );
      elsif State = 0 then
         Item.Value := 0;
         State      := 8;
   ...
   begin
      ...
      Value :=
         (  Shift_Left (Unsigned_32 (Data (Pointer + 3)), 24)
         or Shift_Left (Unsigned_32 (Data (Pointer + 2)), 16)
         or Shift_Left (Unsigned_32 (Data (Pointer + 1)), 8)
         or             Unsigned_32 (Data (Pointer))
         );
      Pointer := Pointer + 4;
   end Get;
   ...
   begin
      ...
      Value :=
         (  Shift_Left (Unsigned_64 (Data (Pointer + 7)), 56)
         or Shift_Left (Unsigned_64 (Data (Pointer + 6)), 48)
         or Shift_Left (Unsigned_64 (Data (Pointer + 5)), 40)
         or Shift_Left (Unsigned_64 (Data (Pointer + 4)), 32)
         or Shift_Left (Unsigned_64 (Data (Pointer + 3)), 24)
         or Shift_Left (Unsigned_64 (Data (Pointer + 2)), 16)
         or Shift_Left (Unsigned_64 (Data (Pointer + 1)), 8)
         or             Unsigned_64 (Data (Pointer))
         );
      Pointer := Pointer + 8;
   end Get;
   ...
   begin
      ...
      Data (Pointer + 1) := Stream_Element (Shift_Right (Value, 8));
      Pointer := Pointer + 2;
   end Put;
...
end GNAT.Sockets.Connection_State_Machine.Little_Endian.Unsigneds;
